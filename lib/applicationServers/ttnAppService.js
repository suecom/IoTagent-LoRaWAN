	...
	preProcessMessage(mqttTopic, message) {
		...
		if (splittedMqttTopic.length !== 5) {
			...
			if (message) {
				...
				} else if (message.end_device_ids) {
					deviceEui = message.end_device_ids.dev_eui;
				}

				if (dataModel === 'application_server' && message.uplink_message) {
					this.messageHandler(this, deviceId, deviceEui, message.uplink_message);
